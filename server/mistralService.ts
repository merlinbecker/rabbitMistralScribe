...
export class MistralService implements IMistralService {
  ...
  private readonly transcriptionUrl = 'https://api.mistral.ai/v1/audio/transcriptions';
  private readonly chatUrl = 'https://api.mistral.ai/v1/chat/completions';

  constructor(config?: { sttModel?: string; chatModel?: string }) {
    this.sttModel = config?.sttModel || process.env.MISTRAL_STT_MODEL || 'voxtral-24.02';
    this.chatModel = config?.chatModel || process.env.MISTRAL_MODEL || 'mistral-large-latest';
  }

  async transcribeAudio(audioBuffer: Buffer, apiKey: string): Promise<TranscriptionResult> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Mistral transcription failed: ${response.statusText} - ${errorText}`);
    }
  ...
  async summarizeText(text: string, apiKey: string, customTemplate?: string): Promise<SummarizationResult> {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Mistral summarization failed: ${response.statusText} - ${errorText}`);
    }
  ...
  async generateTitle(text: string, apiKey: string): Promise<TitleGenerationResult> {
    const systemPrompt = 'Du bist ein Assistent, der prägnante Titel erstellt. Erstelle einen einzeiligen Titel (maximal 60 Zeichen) der die Hauptidee zusammenfasst. Antworte nur mit dem Titel, ohne Anführungszeichen oder zusätzlichen Text.';
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Mistral title generation failed: ${response.statusText} - ${errorText}`);
    }
